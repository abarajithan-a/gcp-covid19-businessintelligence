...
WITH dedup_query AS ( 
	...
	FROM (
		SELECT
			...
			CAST(ct.lat AS FLOAT64) AS city_lat,
			CAST(ct.lng AS FLOAT64) AS city_lng,
			...
			CAST(co.lat AS FLOAT64) AS country_lat,
			CAST(co.lng AS FLOAT64) AS country_lng,
			CAST(cg.gdp_usd AS NUMERIC) AS country_gdp_usd,
			CAST(cl.landarea_sq_kms AS NUMERIC) AS country_landarea_sq_kms,
			CAST(cp.total_population AS INT64) AS country_population,
